...
function accountIDFromString(account: string) : AccountID {
  ...
  if (accountSplit.length !== 3) {
    throw new Error("AccountID string must be of the form realmNum.shardNum.accountNum")
  }

  const accountID = new AccountID()

  if (isNaN(parseInt(accountSplit[0], 10)) || isNaN(parseInt(accountSplit[1], 10)) || isNaN(parseInt(accountSplit[2], 10))) {
