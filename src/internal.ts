...
function friendlyHederaResponseCodeEnum(value: number): string {
    const keyname = enumKeyByValue(value)
    const keynameArray = keyname.split('_')
    const processedArray = keynameArray
        .map(k => k.toLowerCase())
        .map(k => capitalize(k))
